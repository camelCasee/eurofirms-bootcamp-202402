...
function renderProducts() {
    ...
    productsContainer.insertAdjacentElement('beforebegin', productsCounter)

    products.toReversed().forEach(function (product) {
...
createProductForm.onsubmit = function (event) {
    ...
    var titleValidation = fieldValidator(title, 5, 'Title')
    ...
    var imageValidation = fieldValidator(image, 10, 'Image')
    ...
    var imageAltValidation = fieldValidator(imageAlt, 5, 'Image alt')
    if (!imageAltValidation.isValid) {
        alert(imageAltValidation.message)

        return
    }

    var descriptionValidation = fieldValidator(description, 5, 'Description')
    ...
    var priceValidation = fieldValidator(price, 2, 'Price')
    ...
    // var isValidImage = stringValidator(image, 'Image') && lengthValidator(image, 10, 'Image')
